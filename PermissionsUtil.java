    ...
    public static void checkAndRequestPermissions(final Activity activity) {
        ...
        if (denidPermissions.length > 0) {
            ...
            for (String permission : denidArray) {
                ...
                if (!showRationaleUI(activity, permission) && permission.equals(Manifest.permission.ACCESS_FINE_LOCATION)) {
                    // 判断App是否是首次启动
                    if (!isAppFirstRun(activity)) {
                        Snackbar snackbar =
                                Snackbar.make(activity.findViewById(R.id.bottom_navigation_bar), R.string.tips_allow_permission,
                                        Snackbar.LENGTH_LONG);
                        snackbar.setAction(R.string.set, new View.OnClickListener() {
                            @Override
