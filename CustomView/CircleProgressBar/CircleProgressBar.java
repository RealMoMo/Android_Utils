...
public class CircleProgressBar extends View {
    ...
    private void initAttrs(AttributeSet attrs, int defStyle) {
        TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.CircleProgressBar, defStyle, 0);
        mMaxProcess = a.getInt(R.styleable.CircleProgressBar_wheel_max_process, 100);
        mCurProcess = a.getInt(R.styleable.CircleProgressBar_wheel_cur_process, 0);
        if (mCurProcess > mMaxProcess) mCurProcess = mMaxProcess;
        mReachedColor = a.getColor(R.styleable.CircleProgressBar_wheel_reached_color, getColor(R.color.def_reached_color));
        mUnreachedColor = a.getColor(R.styleable.CircleProgressBar_wheel_unreached_color,
                getColor(R.color.def_wheel_color));
        mUnreachedWidth = a.getDimension(R.styleable.CircleProgressBar_wheel_unreached_width,
                getDimen(R.dimen.def_wheel_width));
        isHasReachedCornerRound = a.getBoolean(R.styleable.CircleProgressBar_wheel_reached_has_corner_round, true);
        mReachedWidth = a.getDimension(R.styleable.CircleProgressBar_wheel_reached_width, mUnreachedWidth);
        mPointerColor = a.getColor(R.styleable.CircleProgressBar_wheel_pointer_color, getColor(R.color.def_pointer_color));
        mPointerRadius = a.getDimension(R.styleable.CircleProgressBar_wheel_pointer_radius, mReachedWidth / 2);
        isHasWheelShadow = a.getBoolean(R.styleable.CircleProgressBar_wheel_has_wheel_shadow, false);
        mDrawable = a.getDrawable(R.styleable.CircleProgressBar_wheel_center_img);

        if (isHasWheelShadow) {
            mWheelShadowRadius = a.getDimension(R.styleable.CircleProgressBar_wheel_shadow_radius,
                    getDimen(R.dimen.def_shadow_radius));
        }
        isHasPointerShadow = a.getBoolean(R.styleable.CircleProgressBar_wheel_has_pointer_shadow, false);
        if (isHasPointerShadow) {
            mPointerShadowRadius = a.getDimension(R.styleable.CircleProgressBar_wheel_pointer_shadow_radius,
                    getDimen(R.dimen.def_shadow_radius));
        }
        isHasCache = a.getBoolean(R.styleable.CircleProgressBar_wheel_has_cache, isHasWheelShadow);
        isCanTouch = a.getBoolean(R.styleable.CircleProgressBar_wheel_can_touch, true);
        isScrollOneCircle = a.getBoolean(R.styleable.CircleProgressBar_wheel_scroll_only_one_circle, false);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float right = width - getPaddingRight() - mUnreachedWidth / 2;
        float bottom = height - getPaddingBottom() - mUnreachedWidth / 2;
        float centerX = (left + right) / 2;
        float centerY = (top + bottom) / 2;

        float wheelRadius = (width - getPaddingLeft() - getPaddingRight()) / 2 - mUnreachedWidth / 2;
        ...
        if(mDrawable !=null){
            ...
            mDrawable.setBounds(width/4,height/4,width/4*3,height/4*3);
    ...
    public interface OnSeekBarChangeListener {
        void onChanged(CircleProgressBar seekbar, int curValue);
    }
}
