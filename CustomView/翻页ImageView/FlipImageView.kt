...
import com.realmo.view.R



class FlipImageView(context: Context, attributes: AttributeSet) : View(context, attributes) {
    ...
    private var currentDis : Float = 0f
        ...
        event?.apply {
            when (this.actionMasked) {
                ...
                MotionEvent.ACTION_MOVE -> {
                    ...
                    currentDis = currentY - startY
                    if (Math.abs(currentDis) > 1f && Math.abs(currentDis) >= Math.abs(currentX - startX) * 1.5f) {
                        if (statusFlip == 0) {
                            //滑动间距为正并且不是第一页判断为向下翻，滑动间距为负并且不是最后一页判断为向上翻
                            statusFlip = if (currentDis > 0 && curPage != 0) DOWN_FLIP
                            else if (currentDis < 0 && curPage != girls.lastIndex) UP_FLIP else 0
                        }
                        val ratio = Math.abs(currentDis) / (centerY * standDisRatio)
                        if (statusFlip == DOWN_FLIP) {
                            //向下翻并且当前页不等于0
                            rotateF = ratio * -180f
                            Log.d("realmo", ": rotateF : " + rotateF);
                            invalidate()
                        } else if (statusFlip == UP_FLIP) {
                            //向上翻，并且不是最后一页
                            if (curPage != girls.lastIndex) {
                                rotateS = ratio * 180f
                                Log.d("realmo", ": rotateS : " + rotateS);
                                invalidate()
                            }
                        }
                    }
