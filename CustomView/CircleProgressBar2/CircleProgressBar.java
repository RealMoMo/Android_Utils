...
public class CircleProgressBar extends View {
    ...
    //Draw progressbar text enable
    private boolean mProgressTextEnable;
    ...
    private void initFromAttributes(Context context, AttributeSet attrs) {
        ...
        mProgressTextEnable = a.getBoolean(R.styleable.CircleProgressBar_progress_text_enable, false);
    ...
    private void drawProgressText(Canvas canvas) {
        ...
        progressText = mProgressFormatter.format(mProgress, mMax);
    ...
    private void drawLineProgress(Canvas canvas) {
        unitDegrees = (float) (2.0f * Math.PI / mLineCount);
        //float outerCircleRadius = mRadius;
        //float interCircleRadius = mRadius - mLineWidth;

        progressLineCount = (int) ((float) mProgress / (float) mMax * mLineCount);

        for (int i = 0; i < mLineCount; i++) {
            rotateDegrees = i * -unitDegrees;

            startX = mCenterX + (float) Math.cos(rotateDegrees) * (mRadius - mLineWidth);
            startY = mCenterY - (float) Math.sin(rotateDegrees) * (mRadius - mLineWidth);

            stopX = mCenterX + (float) Math.cos(rotateDegrees) * mRadius;
            stopY = mCenterY - (float) Math.sin(rotateDegrees) * mRadius;
    ...
    private void drawSolidProgress(Canvas canvas) {
        if (mDrawBackgroundOutsideProgress) {
            startAngle = MAX_DEGREE * mProgress / mMax;
            sweepAngle = MAX_DEGREE - startAngle;
